...
helideck_area = st.sidebar.slider("Solar Collector Area (m²)", 10, 100, 50)
...
# Plot results
if show_large:
    st.pyplot(plot_map(net_saving, "Energy per day from solar collector (kWh)", "YlGnBu", large=True))
else:
    col1, col2 = st.columns(2)
    with col1:
        st.pyplot(plot_map(net_saving, "Energy per day from solar collector (kWh)", "YlGnBu"))
        st.pyplot(plot_map(ghi, "Global hor. irradiance (kWh/m²/day)", "Oranges"))
    with col2:
